---
title: "constitutive — does the act posit a social object, or serve a reader"
force: explain
intention: formative
view: synchronic
...
The theory grounds the distinction; reifying it as a stored mark is ours.

**Why we reified it.**
It re-founds `register` honestly.
The `know/do/govern` triad looked like three directions of fit, but "govern" is not a third direction — it is *markedness*.
...
Derive from `force` alone — rejected: `recommend`/`instruct` swing with context.
Derive from `force × power` now — deferred, not rejected.
Keep "govern" as a register primitive — rejected: ill-founded.

**The work it does.**
Founds the `register` gloss; flags the acts an audit or a governance process must track; feeds the ADR-tiering test (a constitutive-durable act earns a record).
