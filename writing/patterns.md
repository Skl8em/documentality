...
These are **recommendations, not rules** (the binding ones are in [`rules.md`](rules.md)).
They lower cognitive load whatever the force.
They apply most strongly when `reader: H+M`; a pure-`H` document may relax the `M>H` items, a pure-`M` document may relax the human-scan ones.
Override any of them knowingly when a document has a good reason.
...
- **No cross-section anaphora** `M>H` — "this value", "the module above" break when a chunk is extracted alone.
  Name the referent.
...
*(The one pattern promoted to a binding rule — strict terminological consistency — lives in [`rules.md`](rules.md).
Full academic references in `../theorising/patterns-ecriture-documentation.md`, web-verified set: Sweller 1988, Cowan 2001, Liu et al. 2024 "Lost in the Middle", Nielsen F-pattern, OASIS DITA 1.3, Procida Diátaxis, PEP 257, Anthropic multishot.)*

## Overriding

These are defaults.
A `reader: H` runbook may drop the recaps; a poster-sized `orient` may ignore working-memory grouping.
Deviate when the document's force and audience justify it — and, if the deviation is systematic, record it in a project `recommend` doc so it is a shared default rather than a one-off.
